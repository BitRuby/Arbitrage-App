...
@Component({
  selector: 'app-book-usdt-zec',
  templateUrl: './book-usdt-zec.component.html',
  styleUrls: ['./book-usdt-zec.component.css']
})
export class BookUsdtZecComponent implements OnInit, AfterViewInit {
    ...
    this.ob.getOrderBook(this.marketId).subscribe(ret => {
      if (ret === null) { return; }
        for (let i = 0; i < 3; i++) {
          ...
          this.dataSource[i + 3] = new MatTableDataSource(ret[i].data.Bids);
          this.dataSource[i + 3].sort = this.sort.toArray()[i + 3];
          this.dataSource[i + 3].paginator = this.paginator.toArray()[i + 3];
        }
    });
  }
}
